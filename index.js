...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Comendo sua mãe, aguarde... ⌛',
				success: '✔️ Deu certo, ufa kk ✔️',
				error: {
					stick: '⚠️ Falha, ocorreu um erro ao converter a imagem em figurinha, tente novamente⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					...
					Badmin: '❌ Este comando só pode ser usado quando o bot virar adm fdp!❌'
				}
			}
			...
			case 'play':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'gasosa'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'sua mãe sabe disso?'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'Comando não esta registrado no script z', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
